...
def gen_prototypes(x_train, y_train):
    # Gerando os protótipos para poder fazer o prodecedimento do LVQ3
    ...
    for _ in range(num_of_prot_training_epochs):
        ...
        for i in range(len(x_train)):
            ...
            # Atualizando os protótipos baseados na distância, igual ao lvq2, porém adicionando o caso das classes iguais
            if is_inside_window:
                if pi_class != pj_class:
                    update_prototype(pi, x_instance, pi_class == x_class, weight)
                    update_prototype(pj, x_instance, pj_class == x_class, weight)
                else:
                    update_prototype(pi, x_instance, True, weight, epslon)
                    update_prototype(pj, x_instance, True, weight, epslon)
